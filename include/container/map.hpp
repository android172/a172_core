 ...
 * @brief Defines common balanced tree container
 * @version 0.1
 * @date 2024-05-24
 *
...
class Map : public std::map<_Key, _Tp, _Compare, TAllocator<_Tp>> {
  ...
  public:
     ...
     */
    Map() : Map(allocator_type(BaseMemoryTags.Map)) {}
    ...
    explicit Map(
        ...
        const allocator_type& __a = allocator_type(BaseMemoryTags.Map)
    )
    ...
    Map(const default_version& __x)
        : _base_class(allocator_type(BaseMemoryTags.Map)) {
        for (auto var : __x)
            insert(var);
    }
