 ...
 * @brief Defines common double link list container
 * @version 0.1
 * @date 2024-05-24
 *
...
class List : public std::list<_Tp, TAllocator<_Tp>> {
  ...
  public:
     ...
     */
    List() : _base_class(allocator_t(BaseMemoryTags.List)) {}
     ...
     */
    explicit List(
        uint64 __n, const allocator_t& __a = allocator_t(BaseMemoryTags.List)
    )
        : _base_class(__n, __a) {}
    ...
    List(
        uint64             __n,
        const _Tp&         __value,
        const allocator_t& __a = allocator_t(BaseMemoryTags.List)
    )
    ...
    List(const default_version& __x)
        : _base_class(allocator_t(BaseMemoryTags.List)) {
        for (auto var : __x)
            push_back(var);
    }
