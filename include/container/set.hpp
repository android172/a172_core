 ...
 * @brief Defines common set container
 * @version 0.1
 * @date 2024-05-24
 *
...
class Set : public std::set<_Key, _Compare, TAllocator<_Key>> {
  ...
  public:
     ...
     */
    Set() : _base_class(allocator_type(BaseMemoryTags.Set)) {}
    ...
    explicit Set(
        ...
        const allocator_type& __a = allocator_type(BaseMemoryTags.Set)
    )
    ...
    Set(_InputIterator __first, _InputIterator __last)
        : _base_class(allocator_type(BaseMemoryTags.Set)) {
     ...
     */
    Set(default_version&& __x) : _base_class(BaseMemoryTags.Set) {
        for (auto var : __x)
            insert(var);
    }
