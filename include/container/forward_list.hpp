 ...
 * @brief Defines common forward list container
 * @version 0.1
 * @date 2024-05-24
 *
...
class ForwardList : public std::forward_list<_Tp, TAllocator<_Tp>> {
  ...
  public:
     ...
     */
    ForwardList() : _base_class(allocator_t(BaseMemoryTags.List)) {}
    ...
    explicit ForwardList(
        uint64 __n, const allocator_t& __al = allocator_t(BaseMemoryTags.List)
    )
    ...
    ForwardList(
        uint64             __n,
        const _Tp&         __value,
        const allocator_t& __al = allocator_t(BaseMemoryTags.List)
    )
