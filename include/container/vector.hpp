...
class Vector : public std::vector<Tp, TAllocator<Tp>> {
  ...
  public:
     ...
     */
    Vector(const TAllocator<Tp>& __a = t_allocator_type(BaseMemoryTags.Array))
        : _base_class(__a) {
    ...
    explicit Vector(
        uint64                __n,
        const TAllocator<Tp>& __a = t_allocator_type(BaseMemoryTags.Array)
    )
        : _base_class(__n, __a) {}
    ...
    Vector(
        uint64                  __n,
        const Tp&               __value,
        const t_allocator_type& __a = t_allocator_type(BaseMemoryTags.Array)
    )
    ...
    Vector(const default_version& __x)
        : _base_class(t_allocator_type(BaseMemoryTags.Array)) {
        reserve(__x.size());
        for (auto var : __x)
